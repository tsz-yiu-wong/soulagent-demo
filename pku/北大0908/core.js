...
(function (global) {
  ...
  const presetCloneTemplates = [
    ...
    },
    {
      ...
      name: "黄铁军",
      avatar: "黄",
      gradient: "linear-gradient(135deg, #8c171e, #b91c1c)",
      role: "教授 · 长江特聘 · 国家杰青",
      dept: "信息与工程科学部 · 计算机学院",
      titles: ["教授", "长江特聘", "国家杰青"],
      researchFields: ["视觉信息处理与神经形态计算"],
      tag: "视觉信息处理与神经形态计算",
      desc: "长期从事视觉信息处理与类脑计算研究，现任北京智源人工智能研究院理事长。",
      status: "ready",
      progress: 100,
      createTime: "2026-08-21",
      files: [
        { id: 'cf6', name: '类脑计算与脉冲神经网络前沿综述_2026.pdf', size: '4.8 MB', time: '2026-08-21 10:00', status: '解析完成', type: 'pdf', tokens: '36,000' },
        { id: 'cf7', name: '数字视网膜与视觉信息处理研究进展.json', size: '2.6 MB', time: '2026-08-21 14:30', status: '解析完成', type: 'json', tokens: '21,500' }
      ],
      settings: {
        systemPrompt: "你是黄铁军AI分身，北京大学教授、北京智源人工智能研究院理事长。长期从事视觉信息处理、类脑计算、神经形态智能与通用人工智能研究，擅长从前沿算法、算力体系到产业战略层面给出专业、前瞻的判断。",
        model: "Soul-LLM-v4-Ultra",
        temperature: 0.75,
        maxTokens: 4096,
        capabilities: { codeInterpreter: false, webSearch: true, deepReasoning: true, memoryPersistence: true }
      ...
      memories: [
        { id: 'cm5', category: '研究方向', content: '聚焦视觉信息处理、类脑计算与通用人工智能，强调理论、算法、算力与数据的协同创新。', time: '2026-08-21 16:00', importance: '高' }
      ]
    }
  ];
  ...
  const defaultExpertList = [
    ...
    },
    {
      ...
      role: "助理教授 · 研究员 · 博士生导师",
      dept: "信息与工程科学部 · 计算机学院",
      titles: ["助理教授", "研究员", "博士生导师"],
      researchFields: ["机器学习与计算机视觉"],
      tag: "机器学习与计算机视觉",
      gradient: "linear-gradient(135deg, #0f766e, #14b8a6)",
      desc: "聚焦机器学习与计算机视觉，从事视觉信息处理与类脑智能研究。",
      stat: "1.6k+ 次对话"
    }
  ];
      ...
      messages: [
        ...
        },
        {
          ...
          htmlContent: `
            ...
            <div class="expert-quote-block" style="background:var(--primary-light, #fdf2f2);border-left:3px solid var(--primary-color, #8c171e);padding:10px 14px;border-radius:0 8px 8px 0;margin:8px 0;color:#334155;">
              <p style="margin:0 0 6px 0;font-weight:600;color:var(--primary-color, #8c171e);">💡 张院士 的建议：</p>
  ...
  class DataStore {
    constructor() {
      ...
      this._syncPresetClonePresentation();
    }

    _syncPresetClonePresentation() {
      const templates = this.presetTemplates || [];
      let changed = false;
      templates.forEach(tpl => {
        if (!tpl.researchFields) return;
        const idx = this.myClones.findIndex(c => c.id === tpl.id);
        if (idx !== -1 && !this.myClones[idx].researchFields) {
          this.myClones[idx] = {
            ...this.myClones[idx],
            name: tpl.name,
            avatar: tpl.avatar,
            ...
            dept: tpl.dept,
            titles: tpl.titles,
            researchFields: tpl.researchFields
          };
          changed = true;
        }
      });
    ...
    startAgentBuild(agentName, cloneId, extraMeta = {}) {
      let clone = null;
      if (cloneId) {
      ...
      if (!clone) {
        clone = {
          ...
          desc: extraMeta.desc || '正在根据您的资料构建专属分身',
          status: 'building',
          progress: 0,
        ...
        clone.progress = Math.max(0, clone.progress || 0);
        if (extraMeta.tag) clone.tag = extraMeta.tag;
        if (extraMeta.desc) clone.desc = extraMeta.desc;
      }

      this.save();
      bus.emit('clone:updated', { type: 'building', clone });
      bus.emit('sidebar:refresh');
      return clone;
    }
  ...
  function generateConsultingReply(target, userQuery, isClone) {
    ...
    if (isClone) {
      if (target.id === 'clone-poetry' || targetName.includes('诗')) {
        ...
        quoteContentHtml = `
          <p style="margin:0 0 6px 0;font-weight:600;color:var(--primary-color, #8c171e);">💡 ${escapeHtml(targetName)} 的专业见解：</p>
      ...
      let cloneQuoteHtml = `
        <p style="margin:0 0 6px 0;font-weight:600;color:var(--primary-color, #8c171e);">💡 ${escapeHtml(targetName)} 的建议：</p>
        <div style="line-height:1.6;color:#334155;">${escapeHtml(opinionText)}</div>
      `;
      quoteContentHtml = cloneQuoteHtml;
