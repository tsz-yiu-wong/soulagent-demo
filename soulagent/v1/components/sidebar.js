...
(function (global) {
  ...
  function renderSidebar() {
    ...
    const historyItemsHtml = chats.map(title => {
      ...
      if (isGroup) {
        badgeHtml = `<span style="display:inline-flex;align-items:center;justify-content:center;min-width:26px;height:18px;padding:0 6px;border-radius:4px;background:linear-gradient(135deg, #7c3aed, #4f46e5);color:#fff;font-size:11px;font-weight:600;flex-shrink:0;white-space:nowrap;">群聊</span>`;
      } else if (clone) {
  ...
  styleEl.textContent = `
    .sidebar {
      width: 250px;
      background-color: var(--bg-sidebar, #f9f9fb);
    ...
    .logo-icon svg { width: 16px; height: 16px; stroke: #ffffff; }
    .divider {
      height: 1px;
