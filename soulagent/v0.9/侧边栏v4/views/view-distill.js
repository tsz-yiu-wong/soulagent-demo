...
function renderDistillView() {
  const activeCloneId = state.activeCloneId;
  let clone = myClones.find(c => c.id === activeCloneId);
  if (!clone) {
    clone = myClones.find(c => c.status === 'distilling') || myClones[0];
  }
  const currentAgent = clone ? clone.name : (state.currentAgent || '专属AI分身');
  const gradient = clone ? clone.gradient : "linear-gradient(135deg, #64748b, #475569)";
  const avatar = clone ? (clone.avatar || clone.name.charAt(0)) : currentAgent.charAt(0).toUpperCase();

  return `
    <div class="distill-view">
      <!-- 顶部返回栏 -->
      <div class="distill-nav">
      ...
      <div class="distill-header">
        <div class="distill-title-group">
          <div class="distill-avatar-badge" style="background: ${gradient};">
            ${escapeHtml(avatar)}
          </div>
          <div>
            <div class="distill-title">正在提炼专属分身：${escapeHtml(currentAgent)}</div>
            <div class="distill-subtitle">SoulAgent 专属提炼引擎 · 打造您的个人数字分身</div>
          </div>
        </div>
