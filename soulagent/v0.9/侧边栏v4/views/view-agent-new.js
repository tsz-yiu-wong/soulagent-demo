...
function renderAgentNewView() {
  ...
  const titleText = (agentName === 'MyAgent' || agentName === 'AI助手' || agentName === '默认智能体' || agentName === '默认')
    ? 'TY，今天需要帮你做什么'
  ...
  return `
    <div class="chat-new-view">
      ...
      <div class="input-card">
        <textarea class="chat-textarea" id="chat-input" placeholder="给 ${escapeHtml(agentName)} 发送消息..." oninput="state.chatInputDraft = this.value">${escapeHtml(draftText)}</textarea>
