...
function renderSidebar() {
  ...
  const isPlazaActive = (currentPage === 'plaza.html' || currentPage === 'expert-new.html' || (currentPage === 'profile.html' && !currentId));
  ...
  // 4. 根据 myClones 数量渲染 AI分身 下方的分身二级子项
  ...
  const clonesItemsHtml = displayClones.map((clone, idx) => {
    const isCloneActive = ((currentPage === 'clone-new.html' || currentPage === 'profile.html') && currentId === clone.id);
  ...
  const sidebarHtml = `
    <aside class="sidebar">
      ...
      <div class="nav-group">
        ...
        <!--
        <a class="nav-item-static" href="javascript:void(0);" style="text-decoration:none; cursor:default; margin-top: 4px;">
