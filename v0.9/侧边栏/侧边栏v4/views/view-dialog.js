...
function renderDialogView() {
  ...
  let convData = conv;
  if (!convData) {
    convData = generateDynamicConversation(chatTitle, isTwin, dialogExp, dialogClone);
  }

  // 介绍卡片（仅在独立卡片发起的专家或分身历史对话中展示）
  let introCardHtml = '';
  if (dialogClone) {
    const p1 = `请介绍你在 ${dialogClone.tag || '专属领域'} 的核心专长`;
    const p2 = `基于知识文件，帮我分析关键要点`;
    const p3 = `给出具体的执行与调优方案`;

    introCardHtml = `
      <div class="expert-intro-card clone-intro-card" style="margin-top: 0; margin-bottom: 8px;">
        <div class="intro-avatar-box clone-avatar-box" style="background: ${dialogClone.gradient};">
          <span style="font-size:28px;font-weight:700;color:#fff;">${escapeHtml(dialogClone.avatar || dialogClone.name.charAt(0))}</span>
        </div>
        <div class="intro-meta">
          <div class="clone-intro-header-row" style="display:flex;align-items:center;gap:10px;margin-bottom:4px;">
            <h2 class="intro-name" style="margin-bottom:0;">${escapeHtml(dialogClone.name)}</h2>
            <span class="clone-status-pill ready"><span class="ready-dot"></span>已就绪</span>
          </div>
          <div class="intro-role" style="background:#fce7f3;color:#be185d;">${escapeHtml(dialogClone.tag || '专属AI分身')} · 我的分身</div>
          <p class="intro-desc">${escapeHtml(dialogClone.desc)}</p>
        </div>
    ...
    `;
  } else if (dialogExp && (convData.type === 'twin' || isTwin)) {
  ...
  const messagesHtml = convData.messages.map(msg => {
    if (msg.role === 'user') {
      ...
      if (dialogClone || (convData && convData.type === 'clone')) {
        const c = dialogClone || myClones.find(item => item.id === convData.cloneId) || myClones[0];
        return `
          <div class="msg-row assistant twin-assistant">
            <div class="twin-avatar-box" style="background: ${c.gradient};" title="${escapeHtml(c.name)}">
              <span style="font-weight:700;font-size:14px;color:#fff;">${escapeHtml(c.avatar || c.name.charAt(0))}</span>
            </div>
        ...
        `;
      } else if (convData.type === 'twin' || isTwin) {
        ...
        // 普通 Agent 消息（艾特专家也是 Agent 视角回复，无表头）
        return `
  ...
  // 底部输入栏左侧内容：仅 Master 对话放选择器，分身与专家对话只放上传附件
  const bottomInputLeftHtml = (!dialogExp && !dialogClone && !isTwin)
  ...
  const inputPlaceholder = dialogClone
    ? `给 ${dialogClone.name} 发送消息...`
    : (dialogExp ? `给 ${dialogExp.name}的AI分身 发送消息...` : `给 ${escapeHtml(state.currentAgent)} 发送消息...`);
    ...
    <div class="chat-dialog-view">
      ...
      <div class="dialog-bottom-input">
        <div class="input-card" style="border-radius: 16px; padding: 12px 14px;">
          <textarea class="chat-textarea" id="dialog-chat-input" style="min-height: 48px; max-height: 120px;" placeholder="${escapeHtml(inputPlaceholder)}"></textarea>
...
function generateDynamicConversation(title, isTwin, exp, clone) {
  if (clone) {
    return {
      type: "clone",
      cloneId: clone.id,
      messages: [
        { role: "user", content: `您好${clone.name}，请结合你的知识文件与设定，探讨关于“${title}”的核心思路。` },
        {
          role: "assistant",
          htmlContent: `
            <p>你好！我是你的专属AI分身 <strong>${escapeHtml(clone.name)}</strong>。</p>
            <p>结合我在<strong>【${escapeHtml(clone.tag || '专属领域')}】</strong>的专业语料与人设沉淀，针对<strong>“${escapeHtml(title)}”</strong>，我的核心思路如下：</p>
            <ol>
              <li><strong>专属知识库检索增强</strong>：从本地知识文件中定位核心事实与特征维度。</li>
              <li><strong>精准领域推理</strong>：严格遵循模型参数与逻辑规范输出执行建议。</li>
              <li><strong>持续迭代演进</strong>：结合实时任务反馈不断沉淀长效记忆。</li>
            </ol>
            <p>随时提出具体问题，我们进一步深入协作！</p>
          `
        }
      ]
    };
