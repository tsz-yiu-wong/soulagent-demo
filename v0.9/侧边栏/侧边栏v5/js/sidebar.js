...
function renderSidebar() {
  ...
  const displayClones = clones.slice(0, 2);
  const clonesItemsHtml = displayClones.map((clone, idx) => {
    const isCloneActive = (currentPage === 'clone-new.html' && (currentId === clone.id || (!currentId && idx === 0)));
  ...
  const viewAllHtml = `
    <a class="sub-branch-item view-all ${isClonesActive ? 'active' : ''}" href="${clonesUrl}" style="text-decoration:none;">
      <span class="branch-icon-box">
        <svg class="icon" viewBox="0 0 24 24" style="width:13px;height:13px;stroke:currentColor;"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>
      </span>
      <span>查看全部</span>
    </a>
  `;
  ...
  if (clonesCount === 0) {
    ...
    subClonesHtml = clonesItemsHtml + createCloneHtml + viewAllHtml;
  }
  ...
  const sidebarHtml = `
    <aside class="sidebar">
      ...
      <div class="nav-group" style="margin-top: 4px;">
        ...
        <a class="nav-item-static ${isScheduleActive ? 'active' : ''}" href="${scheduleUrl}" style="text-decoration:none;">
